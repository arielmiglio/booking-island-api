...
public class DateRangeValidator implements ConstraintValidator<ValidateDateRange, Object> {
	...
	if (arrival == null || departure == null) {
	    ...
	    context.buildConstraintViolationWithTemplate("Arrival and Departure should be indicated")
	...
	if (arrival.isAfter(departure) || arrival.isEqual(departure)) {
	    context.disableDefaultConstraintViolation();
	    context.buildConstraintViolationWithTemplate("Arrival date must be greater than departure date")
		    .addConstraintViolation();
	    return false;
	}
